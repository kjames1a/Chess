...
public class QueenMoves {
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition myPosition) {
        ...
        queenMovesCapture(myPosition, chessPieceMoves, 1, 1, board);
        queenMovesCapture(myPosition, chessPieceMoves, -1, -1, board);
        queenMovesCapture(myPosition, chessPieceMoves, 1, -1, board);
        queenMovesCapture(myPosition, chessPieceMoves, -1, 1, board);
        queenMovesCapture(myPosition, chessPieceMoves, 1, 0, board);
        queenMovesCapture(myPosition, chessPieceMoves, -1, 0, board);
        queenMovesCapture(myPosition, chessPieceMoves, 0, 1, board);
        queenMovesCapture(myPosition, chessPieceMoves, 0, -1, board);
    ...
    private void queenMovesCapture (ChessPosition myPosition, Collection<ChessMove> chessMoves, int rowMove, int colMove, ChessBoard board) {
        int row = myPosition.getRow();
        int col = myPosition.getColumn();
